...
start = datetime(1993,4,8,0)
end = datetime(1993,7,20,0)
...
vfs = 0.3 # fraction of very fine sand in the surface soil 
...
KIADJ = 0.2 * np.ones(len(RCF))
KRADJ = 0.2 * np.ones(len(RCF))
SHCRTADJ = np.ones(len(RCF))
detention = 0.0002 # detension storage for deposition term [m]
...
# np.save('source_1_event1.npy', source_1)
# np.save('source_2_event1.npy', source_2)
# np.save('source_3_event1.npy', source_3)
# np.save('source_4_event1.npy', source_4)
# np.save('source_5_event1.npy', source_5)

source_1 = np.load('source_1_event1.npy')
source_2 = np.load('source_2_event1.npy')
source_3 = np.load('source_3_event1.npy')
source_4 = np.load('source_4_event1.npy')
source_5 = np.load('source_5_event1.npy')
...
# =============================================================================
# Comparisons with sediment observations
# =============================================================================
os.chdir(path)
...
ax.set_xticklabels([])
plt.text(dti[0],80, 'NSE = '+str(np.round(NSE_Q,2)),fontsize=12)
plt.legend()
...
ax.set_xticklabels(dti[::interval])
ax.set_xticklabels([])
plt.text(dti[0],450, 'NSE = '+str(np.round(NSE,2)),fontsize=12)
plt.legend()
...
ax.set_xticks(dti[::interval])
ax.set_xticklabels(dti[::interval])
plt.text(dti[0],1300, 'PBIAS = '+str(np.round(p_bias,2))+'%',fontsize=12)
plt.tight_layout()
plt.savefig('event1_WRE6.png',dpi=300)
plt.show()
...
# sediment yield [kg] after transport
s_l = np.zeros([4,10]) 
s_l = (load_1[n,:,:] +load_2[n,:,:]+load_3[n,:,:]\
        +load_4[n,:,:] +load_5[n,:,:] )  

plt.subplot(221) # water depth [mm]
plt.pcolormesh(X, Y, H[n,:,:]*1000, cmap=plt.cm.jet, vmin=0, vmax=np.max(H[n,:,:]*1000))
plt.colorbar()
plt.title('(a) Water depth [mm]')
plt.xlabel('x [m]')
plt.ylabel('y [m]')


plt.subplot(222) # sediment production [kg]
plt.pcolormesh(X, Y, s_p[n,:,:], cmap=plt.cm.jet, \
                vmin=0, vmax=np.max(s_p[n,:,:]))
plt.colorbar()
plt.title('(b) Source [kg]')
plt.xlabel('x [m]')
plt.ylabel('y [m]')

# sediment deposit [kg]
daily_dep = (dep_1+dep_2+dep_3+dep_4+dep_5) * dx * dy * 3600 * 24 # (kg)
plt.subplot(223)
plt.pcolormesh(X, Y, daily_dep[n+1,:,:], cmap=plt.cm.jet, \
                vmin=0, vmax=np.max(daily_dep[n+1,:,:]))
plt.colorbar()
plt.title('(c) Sink [kg]')
plt.xlabel('x [m]')
plt.ylabel('y [m]')

plt.subplot(224) # sediment load [kg]
plt.pcolormesh(X, Y, s_l, cmap=plt.cm.jet, \
                vmin=0, vmax=np.max(s_l))
plt.colorbar()
plt.title('(d) Sediment yield [kg]')
plt.xlabel('x [m]')
plt.ylabel('y [m]')
plt.tight_layout()
plt.savefig('5_9_1993_WRE6.png',dpi=300)
plt.show()

# sediment production vs. sediment yield
sp_n = np.zeros(nt)
for n in range(nt):
    sp_n[n] = ( np.sum(source_1[n,:,:]) + np.sum(source_2[n,:,:]) +np.sum(source_3[n,:,:])\
        +np.sum(source_4[n,:,:]) +np.sum(source_5[n,:,:])) * dx * dy * 3600 * 24 
sp_n_cum = np.cumsum(sp_n)
plt.plot(dti,sp_n_cum, label='Production')
plt.plot(dti,sed_cum_simul, label='Yield')
plt.ylabel('Sediment [kg]')
ax = plt.gca()
ax.set_xticks(dti[::interval])
ax.set_xticklabels(dti[::interval])
...
plt.tight_layout()
plt.legend()
plt.savefig('production_yield_WRE6.png',dpi=300)
plt.show()
